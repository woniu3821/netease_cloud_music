        ...
        contentWidget: <Widget>[
          ListView(
            ...
            children: video.videos
                .map((video) => SearchVideoWidget(
                      url: video.coverUrl,
                      playCount: video.playTime,
                      title: video.title,
                      type: video.type,
                      creatorName:
                          video.creator.map((c) => c.userName).join('/'),
                    ))
                .toList(),
          ),
        ],
...
// 构建更多文字
  ...
  Widget _buildSimQuery(SimQuery sim) {
    return sim == null || sim.sim_querys.isEmpty
        ? Container()
        : _buildModuleTemplate('相关搜索', contentWidget: <Widget>[
            Wrap(
