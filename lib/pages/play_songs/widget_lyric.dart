...
class LyricWidget extends CustomPainter with ChangeNotifier {
  ...
  set offsetY(double value) {
    ...
    if (isDragging) {
      ...
      if (_offsetY.abs() < lyricPaints[0].height + ScreenUtil().setWidth(30)) {
        ...
        //不能大于最大位置
        _offsetY = (totalHeight +
            lyricPaints[0].height +
            ScreenUtil().setWidth(30) * -1);
      } else {
        _offsetY = value;
      }
    } else {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < lyric.length; i++) {
      if (y > size.height || y < (0 - lyricPaints[i].height / 2)) {
        ...
        lyricPaints[i]
            .paint(canvas, Offset((size.width - lyricPaints[i].width) / 2, y));
      }
      //计算偏移量
  ...
  void _layoutTextPainters() {
    ...
    Future.delayed(Duration(milliseconds: 300), () {
      totalHeight = (lyricPaints[0].height +
          ScreenUtil().setWidth(30) * (lyricPaints.length - 1));
    });
  }
